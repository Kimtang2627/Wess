...
export default function Box ({background , picture , hasLogo ,location , textColor , description}: {background : string , picture : string , hasLogo : boolean , location : string , textColor : string , description : string})  {
    return (
        <>
        <div className={style.wrapper} style={{background : background}} >
            <img className={style.picture} src={`${picture}`} alt="Image" />
            {hasLogo && <img className={style.logo} src="../newspaper.svg" alt="Logo" /> }
